...
const CardAdmin = ({element}) => {
    ...
    if(element['@type'] == 'Experience'){
        url = '/edition-experience/';
        name = "l'experience";
        nameApi = ExperienceApi;
    }
    else if (element['@type'] == 'Projects'){
        url = '/edition-du-projet/'
        name = "le projet";
        nameApi = ProjectApi;
    }
    else if (element['@type'] == 'Education'){
        url = '/edition-formation/'
        name = "la formation";
        nameApi = EducationApi;
    }
    ...
    const archiveProject = (id) => {
        Swal.fire({
            title: `<strong>Archiver ${name}</strong>`,
            ...
            if (result.isConfirmed) {
                Swal.fire({
                    ...
                    title: 'Archivé!',
                    html: `${name} a été archivé avec succès!`,
                    icon: 'success',
    ...
    const deleteProject = (event, id) => {
        Swal.fire({
            title: `<strong>Supprimer ${name}</strong>`,
            html: '<p>Êtes-vous sûre?</p>',
            icon: "warning",
            showCancelButton: true,
            confirmButtonColor: '#e10404',
            ...
            if (result.isConfirmed) {
                ...
                Swal.fire({
                    ...
                    title: 'Supprimé!',
                    html: `${name} a été supprimé avec succès!`,
                    icon: 'success',
                    confirmButtonColor: '#000'
                });
            }
        ...
        <div className="cardAdmin">
            <div className="wrapper" style={{backgroundImage: `url(${BASE_URL + thumb.path})`}}>
                <div className="date">
                    ...
                    <span className="year">{new Date(element.dateOfCreation).getFullYear()}</span>
                </div>
                <div className="data">
                    <div className="content">
                        <span className="author">{element.company}</span>
                        <h1 className="title"><a href="/">{element.name}</a></h1>
                        <p className="text">{element.description}</p>
                        <label htmlFor={`show-menu${element.id}`} className="menu-button"><span></span></label>
                    </div>
                    <input type="checkbox" id={`show-menu${element.id}`} />
                    <ul className="menu-content">
                        ...
                        <li>
                            <button onClick={event => deleteProject(event, element.id)}>
